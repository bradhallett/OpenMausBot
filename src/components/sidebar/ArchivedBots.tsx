...
export function ArchivedBotsPanel({
  ...
  // A real dialog keeps Tab inside and hands focus back to the opener when it
  // closes; the capture runs once so toggling the busy locks never recalls the
  // opener mid-session. The Plus menu item that opens this panel unmounts
  // itself in the same click, so restore through the caller's returnFocusRef
  // (the still-mounted Plus trigger) before falling back to the captured
  // element.
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const focusable = Array.from(
        panel.querySelectorAll<HTMLElement>("button:not([disabled]), [href], input:not([disabled]), select, textarea"),
      );
