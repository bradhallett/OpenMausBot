...
it("bounds a real roster, retrieves its tail, isolates owners and expires stopped capabilities", async () => {
  ...
  try {
    ...
    // Live content-class boundary (#1670): configure the bot mid-flight, save
    // a mixed-class payload, and verify what the model pages back plus the
    // escape-hatch audit in the canonical thread log.
    const configured = await api(`/api/bots/${bot.id}`, { contentClasses: ["personal"] }, undefined, "PATCH");
    expect(configured.status).toBe(200);
    expect(configured.body.bot.contentClasses).toEqual(["personal"]);
    const mixed = await api("/api/internal/tool-result", {
      text: "owner jane@example.com host corp.internal key sk-ant-" + "z".repeat(40),
    }, first.token);
    expect(mixed.status).toBe(201);
    const masked = await api(`/api/internal/tool-result?id=${mixed.body.id}`, undefined, first.token);
    expect(masked.status).toBe(200);
    expect(masked.body.text).not.toContain("jane@example.com");
