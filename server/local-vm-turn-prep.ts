...
// module-level by-value consumer of readyLocalVmForTurn; broadcast arrives
// as a thunk over a const index.ts declares after that site, the mutable
// localVmProvisionBusy flag as a get/set pair over a let the routes and the
// maintenance idle gate also read and write, and the lifecycle values come
// from the createComputerLifecycle destructure above it.
 ...
 * also read and write, crossing as a get/set pair); the lifecycle family
 * is values from the createComputerLifecycle destructure above the site. */
...
export function createLocalVmTurnPrep(deps: LocalVmTurnPrepDeps) {
  const { broadcast, localVmProvisionBusy } = deps.lateBound;
  const {
  ...
  async function readyLocalVmForTurn(botId: string, target: LocalVmTarget, isCurrent = () => true) {
    ...
    const ownsProvision = !localVmProvisionBusy.get();
    if (ownsProvision) localVmProvisionBusy.set(true);
    ...
    try {
      ...
      if (ownsProvision) localVmProvisionBusy.set(false);
