      ...
      if (
        ...
        if (browser) {
          ...
          browserCapture = async () => {
            if (!claimTurnResource(resourceOwner, `browser:${session}`)) throw new Error("another thread is using this browser");
...
const handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
  ...
  try {
    ...
    if (m) {
      ...
      if (method === "GET" && action !== "control") return json(res, 404, { error: "unknown team computer action" });
