...
function Preview() {
  ...
  useEffect(() => { dispatch({ type: "openOverlay", kind: "appSettings", open: true, section: "engines" }); }, [dispatch]);
  return <>
    ...
    <nav aria-label="Preview controls" className="fixed bottom-1 left-1/2 z-[100] flex max-w-full -translate-x-1/2 gap-1 overflow-x-auto rounded-lg border border-hairline bg-panel p-1 text-[11px] text-ink shadow-sm [&_button]:whitespace-nowrap [&_button]:rounded [&_button]:px-2 [&_button]:py-1 [&_button:hover]:bg-control">
      <button onClick={() => { setOnboarding(false); dispatch({ type: "openOverlay", kind: "appSettings", open: true, section: "engines" }); }}>Settings preview</button>
      <button onClick={() => { dispatch({ type: "closeOverlay", kind: "appSettings" }); setOnboarding(true); }}>Onboarding preview</button>
